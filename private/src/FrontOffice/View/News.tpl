...
<div id="left">
	<div class="grandcadre">
		<h2>Les actualités cinéma, séries TV et stars</h2>
...
<div id="pub">
	...
	<div class="cadre_solo_tier_solo_right">
		<h2 style="border-top:5px solid blue;">Actualités par thèmes</h2>
		<b>Cinéma</b> :<br/>
		{foreach $type['cinema'] as $sType => $sUrl}<a href="{$sUrl}">{$sType}</a> {/foreach}<br/>
		<b>Série TV</b> :<br/>
		{foreach $type['serie'] as $sType => $sUrl}<a href="{$sUrl}">{$sType}</a> {/foreach}<br/>
	</div>
	<div class="cadre_solo_tier_solo_right">
		<h2 style="border-top:5px solid blue;">Derniers dossiers</h2>
		{foreach $folders as $iKey2 => $oNews2}
			<div class="demicadre" style="margin-bottom:20px;" itemscope itemtype="http://schema.org/NewsArticle">
				<div class='imgnew'><a href="{$oNews2->url}" itemprop="url"><img src="/images/article_{$oNews2->get_id()}.jpg" alt="{$oNews2->get_title() addslashes}" title="{$oNews2->get_title() addslashes}" border="0" width="300" itemprop="image"/></a></div>
				<div class='contentnews'>
					<a href="{$oNews2->url}" itemprop="name"><b>{$oNews2->get_title()}</b></a>
		...
		{/foreach}
		<br/>&gt; <a href="{url 'dossier'}">Tous les dossiers</a>
	</div>
	<div class="cadre_solo_tier_solo_right">
