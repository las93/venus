...
<div class="double_tier_left_bloc">
	<div style="float:left;width:440px;marging-bottom:5px;">
		<h1 style="border-bottom:dotted 1px gray;">Actualités de {$record->get_title()}</h1>
	</div>
...
<div class="double_tier_left_bloc" style="margin-top:10px;text-align:center;">
	...
	{if !isset($alias)}{assign var=$alias value='film-news'}{/if}
	...
	{else}
		&lt;&lt; <a href="{url alias=$alias id=$record->get_id() title=$record->title_encode base=$record->base}">début</a> |
		&lt; <a href="{url alias=$alias id=$record->get_id() title=$record->title_encode base=$record->base offset=$offset-1}">précédente</a> |
		{section name=$i loop=$loop start=$start}
			<a href="{url alias=$alias id=$record->get_id() title=$record->title_encode base=$record->base offset=$i}">{$i}</a> |
		{/section}
		<a href="{url alias=$alias id=$record->get_id() title=$record->title_encode base=$record->base offset=$offset+1}">suivante</a> &gt; |
		<a href="{url alias=$alias id=$record->get_id() title=$record->title_encode base=$record->base offset=$news['pages']}">fin</a> &gt;&gt;
	{/if}
</div>