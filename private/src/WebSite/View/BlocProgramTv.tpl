...
<div class="double_tier_left_bloc">
	<h1 style="border-bottom:dotted 1px gray;">{if !isset($h1_programtv)}Programme TV{else}{$h1_programtv}{/if}</h1>
	{foreach from=$program['items'] key=$iKey item=$oChannel}
		<div class="one_news_bloc">
			<div class="img_channel_left_bloc">
				<a href="{$oChannel->url}"><img src="{$app.const.IMG_PROD}/img/logo_{$oChannel->get_id()}.jpg" style="border:0"/></a>
			</div>
			{foreach from=$oChannel->program key=$iKey2 item=$oProgram}
				<div class="text_left_demi_bloc">
						<div class="title_list">
							<b>{$oProgram->get_start()|hour} - {$oProgram->get_end()|hour}</b> <a href="{$oProgram->url}" style="color:black;text-decoration:none;">{$oProgram->title_program}</a><br/>
							[<a href="{$oProgram->url}" style="color:black;text-decoration:none;">Voir la fiche</a>]
						</div>
				</div>
			{/foreach}
		</div>
