{* version=2; *}
<h1 style="border-bottom:dotted 1px gray;">{if !isset($h1_records)}Les meilleurs films au cinéma{else}{$h1_records}{/if}</h1>
{foreach from=$records['items'] key=$iKey item=$oRecord}
...
<div class="one_news_bloc" style="height:20px;padding-top:5px;width:960px;{if isset($paging) && $paging === true}text-align:center;{/if}">
	{if isset($paging) && $paging === true}
		...
		{if !isset($alias)}{assign var=$alias value='meilleurs-films'}{/if}
		...
		{/if}
		{if $records['pages'] > $offset + 8}
			{assign var=$loop value=$offset+8}
		{else}
			{assign var=$loop value=$records['pages']-$start}
		{/if}

		{if $start == 1 && $offset - 8 < 1}
			{assign var=$loop value=$loop-$offset+8}
		{/if}
		{if $loop + $start > $records['pages']}
			{assign var=$loop value=$records['pages']}
		{/if}
		...
		{else}
			...
			<a href="{url alias=$alias offset=$records['pages']}">fin</a> &gt;&gt;
		{/if}
	{else}
