...
use \Venus\src\WebSite\Entity\user as entityAccount;
use \Venus\src\WebSite\Model\user as modelAccount;
...
class Account extends Controller {
	...
	public function create() {
		...
		if ($bValidation == true) {
			...
			$this->cookie
				 ->set('user', $oUserCheck->get_login(), 9000, '/', $_SERVER['HTTP_HOST'])
				 ->set('usertoken', md5($oUserCheck->get_login().$oUserCheck->get_password().$this->_sPrivateKey), 9000, '/', $_SERVER['HTTP_HOST']);

			$this->layout
				 ->assign('title', 'Inscription validée  - iScreenway')
			 	 ->assign('model', 'AccountOk.tpl')
	...
	public function deconnect() {
		...
		$this->cookie
			 ->set('user', '', time() - 42000, '/', $_SERVER['HTTP_HOST'])
			 ->set('usertoken', '', time() - 42000, '/', $_SERVER['HTTP_HOST']);
